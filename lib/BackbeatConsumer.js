    ...
    subscribe() {
        ...
        this._consumer.on('message', entry => {
            ...
            q.push(entry, err => {
                ...
                } else if (entry.topic === 'backbeat-replication') {
                    // TODO PHIL - Need to fix (after status topic gets added)
                    // Since originally we only had 2 topics, this worked fine
                    // But metrics should be incremented here based on proper
                    // replicated entries, including updating versioned entries
                    const qEntry = QueueEntry.createFromKafkaEntry(entry);
                    if (qEntry.getReducedLocations) {
                        ...
                        // TODO PHIL - this will log on bucket creations.
                        // Should silence if not needed
                        this._log.error('queue entry has no function '
                        + 'getReducedLocations', {
                            method: 'BackbeatConsumer.subscribe',
                            key: entry.key,
                            bucket: entry.bucket,
